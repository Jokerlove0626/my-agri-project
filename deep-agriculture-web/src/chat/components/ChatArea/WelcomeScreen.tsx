    ...
    const handleSuggestion = (type: string, text?: string) => {
        ...
        switch(type) {
            ...
            case 'agent':
                ...
                break;
            case 'hot-search':
                prompt = `详细解读一下农业相关热搜 "${text || '农业热点'}"，并结合实际生产给出要点、风险提示与可操作建议`;
                break;
            default:
                prompt = "你好！";
        }
    ...
    return (
        <div className={`welcome-screen ${isHidden ? 'hidden' : ''}`}>
            ...
            <div className="welcome-header">
                {/* 可以替换成 DeepAgriculture 的 Logo SVG */}
                <div className="logo-container" style={{ display: 'flex', justifyContent: 'center', width: '100%' , marginBottom: '5px'}}>
                    <Image src="/deepagriculture.jpg" alt="DeepAgriculture Logo" width={380} height={380} />
                </div>
                <h1>我是 DeepAgriculture，你的农业病虫害智能助手</h1>
                <p>
                    我可以帮你解答农业病虫害问题，
            ...
            <div className="suggestion-cards">
                {/* 今日热搜卡片 (示例，内容可替换为农业相关) */}
                <div className="card today-hot">
                    <h3>农业热点</h3>
                    <p>近期农业领域关注焦点</p>
                    <ul>
                        {/* 示例热搜项，可以动态获取 */}
                        <li onClick={() => handleSuggestion('hot-search', '小麦赤霉病防控关键技术与用药建议')}><span>1</span> 小麦赤霉病防控关键技术与用药建议</li>
                        <li onClick={() => handleSuggestion('hot-search', '水稻纹枯病高发期监测预警与田间管理')}><span>2</span> 水稻纹枯病高发期监测预警与田间管理</li>
                        <li onClick={() => handleSuggestion('hot-search', '玉米草地贪夜蛾发生趋势与综合防治方案')}><span>3</span> 玉米草地贪夜蛾发生趋势与综合防治方案</li>
                        <li onClick={() => handleSuggestion('hot-search', '柑橘黄龙病防控进展与园区检疫要点')}><span>4</span> 柑橘黄龙病防控进展与园区检疫要点</li>
                        <li onClick={() => handleSuggestion('hot-search', '番茄晚疫病识别与抗药性管理实用指南')}><span>5</span> 番茄晚疫病识别与抗药性管理实用指南</li>
                    </ul>
                </div>

                {/* 效率工具卡片 (示例，内容可替换为农业相关) */}
                <div className="card efficiency-tools">
                    <h3>常用功能</h3>
                    <p>快速访问 DeepAgriculture 核心能力</p>
                    <div className="tool-item" onClick={() => handleSuggestion('tool', '识别图片中的病虫害')}>
                        <Icon name="Camera" className="feather" />
                        <div>病虫害识别</div>
                        <span>上传图片，AI 快速识别</span>
                    </div>
                    <div className="tool-item" onClick={() => handleSuggestion('tool', '查询草地贪夜蛾的防治方法')}>
                        <Icon name="Search" className="feather" />
                        <div>知识查询</div>
                        <span>输入名称，获取详细防治方案</span>
                    </div>
                    <div className="tool-item" onClick={() => handleSuggestion('tool', '生成一份关于小麦赤霉病的防治报告')}>
                ...
                <div className="card featured-agent">
                     <h3>知识图谱查询</h3>
                     <div className="agent-item" onClick={() => handleSuggestion('agent', '查询与水稻相关的主要病害有哪些')}>
                        {/* 可以使用农业相关的 Logo 或图标 */}
